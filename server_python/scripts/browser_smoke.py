...
Quan trong: script goi thang `browser_service.fetch_page` — dung ham ma tang
ETL dung that — thay vi tu mo browser rieng. Neu tu mo, no chi bat duoc truong
hop thieu binary; con truong hop ai do sua sang `headless=False` thi van xanh.
Goi ham that bat duoc ca hai. URL dung dang `data:` nen khong cham mang ngoai.
"""
...
async def main() -> int:
    try:
        html, _css = await browser_service.fetch_page(PROBE_URL)
